...
func TestOpenAIUpstreamRateLimitWithoutLocalCodeRemainsUnchanged(t *testing.T) {
	...
	assert.Equal(t, "rate_limit_error", gjson.GetBytes(w.Body.Bytes(), "error.type").String())
	// HFC's OpenAI contract always includes the nullable code field; upstream
	// errors without a local gateway code must leave it null rather than inventing one.
	assert.Equal(t, gjson.Null, gjson.GetBytes(w.Body.Bytes(), "error.code").Type)
}
...
func TestGatewayAdmissionError_MessagesStreamingIncludesGatewayCode(t *testing.T) {
	...
	require.True(t, ok)
	assert.Equal(t, "rate_limit_error", errorObject["type"])
	assert.Equal(t, gatewayConcurrencyLimitCode, errorObject["code"])
	assert.Equal(t, "req_reqtestmessages", payload["request_id"])
	assert.Equal(t, payload["request_id"], w.Header().Get("request-id"))
}
