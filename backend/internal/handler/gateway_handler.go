...
func (h *GatewayHandler) handleStreamingAwareErrorWithCode(c *gin.Context, status int, errType, code, message string, streamStarted bool) {
	if streamStarted {
		...
		if ok {
			payloadObject := middleware2.AnthropicErrorPayloadForStatus(c, status, errType, message)
			if code != "" {
				if errorObject, ok := payloadObject["error"].(gin.H); ok {
...
func (h *GatewayHandler) errorResponseWithCode(c *gin.Context, status int, errType, code, message string) {
	payload := middleware2.AnthropicErrorPayloadForStatus(c, status, errType, message)
	if code != "" {
		if errorObject, ok := payload["error"].(gin.H); ok {
			errorObject["code"] = code
		}
	}
