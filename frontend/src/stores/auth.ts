...
export const useAuthStore = defineStore('auth', () => {
  ...
  const token = ref<string | null>(getSessionAccessToken())
  ...
  const unsubscribeAccessToken = subscribeSessionAccessToken((nextToken) => {
    token.value = nextToken
  })
  onScopeDispose(unsubscribeAccessToken)
  ...
  /**
   * Restore a browser session through the HttpOnly refresh cookie.
   */
  async function checkAuth(): Promise<void> {
    ...
    initializationPromise = (async () => {
      try {
        ...
        setSessionAccessToken(refreshed.access_token)
        scheduleTokenRefresh(refreshed.expires_in)
        await refreshUser()
        startAutoRefresh()
      } catch {
  ...
  async function performTokenRefresh(): Promise<void> {
    try {
      ...
      // Schedule the next in-memory access-token refresh.
  ...
  async function setToken(newToken: string): Promise<User> {
    ...
    tokenExpiresAt.value = authAPI.getTokenExpiresAt()
