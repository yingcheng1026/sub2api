      ...
      <div class="flex items-end justify-between">
        <div>
          <div class="text-xs uppercase tracking-wide text-gray-500 dark:text-dark-400">
            {{ isInDebt
              ? t('userSubscriptions.wallet.debtToCover')
              : t('userSubscriptions.wallet.remaining') }}
          </div>
          <div class="mt-1 flex items-baseline gap-2">
            <span
              class="text-3xl font-bold"
              :class="isInDebt ? 'text-red-600 dark:text-red-400' : 'text-amber-700 dark:text-amber-300'"
            >
        ...
        <div class="flex items-start gap-2">
          ...
          <div class="flex-1">
            <p class="font-medium text-amber-900 dark:text-amber-200">
              {{ isInDebt
                ? t('userSubscriptions.wallet.debtWarning', { amount: debtAmount.toFixed(2) })
                : remaining === 0
...
import { getWalletInitialUSD, getWalletRemainingUSD, getWalletUsedPercent } from '@/utils/subscriptionWallet'
...
const remaining = computed(() => getWalletRemainingUSD(props.subscription))
const initial = computed(() => getWalletInitialUSD(props.subscription))
const usedPercent = computed(() => getWalletUsedPercent(props.subscription))
const progressWidth = computed(() => Math.min(100, Math.max(0, usedPercent.value)))
const isInDebt = computed(() => remaining.value < 0)
const debtAmount = computed(() => Math.max(0, -remaining.value))
const displayedBalance = computed(() => isInDebt.value ? debtAmount.value : Math.max(0, remaining.value))
