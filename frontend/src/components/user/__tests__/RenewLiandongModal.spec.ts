...
describe('RenewLiandongModal', () => {
  it('only presents credit top-up tiers and the redeem flow', () => {
    ...
    expect(text).toContain('联系管理员或客服')
    expect(text).toContain('按客服确认的方式完成转账')
    expect(text).not.toContain('月卡')
    expect(text).not.toContain('自动到账')
    expect(text).not.toContain('微信号')
    expect(text).not.toContain('微信转账')
    expect(text).not.toContain('climb102626')
  })
})
